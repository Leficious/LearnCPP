...
double calculateBallHeight(double towerHeight, int seconds)
{
	...
	const double fallDistance{ physics::gravity * (seconds * seconds) / 2.0 };
...
double calculateAndPrintBallHeight(double towerHeight, int seconds)
{
	...
	return calculateBallHeight(towerHeight, seconds);
}
...
int main()
{
	...
	int numSeconds{};
	while (calculateAndPrintBallHeight(towerHeight, numSeconds) > 0.0){
		++numSeconds;
	}
	return 0;
}